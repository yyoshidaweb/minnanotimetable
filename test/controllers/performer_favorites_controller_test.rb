...
class PerformerFavoritesControllerTest < ActionDispatch::IntegrationTest
  ...
  setup do
    ...
    sign_in @user
    @event = events(:one)
    @favorite_performer = performer_favorites(:user_one_performer_one)
    @not_favorite_performer = @event.performers.second
    # リファラーを設定（お気に入り登録・解除後のリダイレクト先として使用）
    @referer = event_performers_path(@event.event_key)
  end

  test "お気に入り登録できる" do
    assert_difference("PerformerFavorite.count", 1) do
  ...
  test "お気に入り解除できる" do
    assert_difference("PerformerFavorite.count", -1) do
      delete performer_favorite_path(@favorite_performer),
              headers: { "HTTP_REFERER" => @referer }
    end
